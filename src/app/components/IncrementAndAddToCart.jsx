...
export default function IncrementAndAddToCart({ product, initNum, showAdd }) {
  ...
  const handleAddToCart = () => {
    ...
    if (newItem) {
      setCartItems([
        ...cartItems,
        { itemName: product, amountOfItems: amountOfItems },
      ]);
    } else {
    ...
    <>
      <div className={"flex items-center gap-2"}>
        <button
          ...
          className={
            "bg-core text-white rounded-full w-[2rem] h-[2rem] font-bold"
          }
        >
          -
        </button>
      ...
      {showAdd ? (
        <button
          ...
          className={
            "px-5 py-2 text-lg bg-secondary rounded-md hover:bg-accent"
          }
        >
          add to cart
        </button>
